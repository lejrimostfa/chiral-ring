"""Proposition C verification (Paper 3, gate G7): fresh-ring increment
positivity + bond factorization of the local marginal.
...
Ported into src/lattice/ (production engine `lattice.py`) as gate G7. The only
adaptation vs the session prototype is the local_marginal_sigma call signature:
production is local_marginal_sigma(R, G, Tc, param, S_true, i).

Run:  python3 verify_C1.py    (from src/lattice)
"""
import os, sys
import numpy as np
...
for fn in ("build_chain", "all_branches", "beta_matrix", "sample_records",
           "local_marginal_sigma"):
    assert hasattr(eng, fn), (f"engine module lacks {fn}; adapt the import section "
                              f"(names may differ in production lattice.py)")
...
S_true_vec = S_all[0]                        # (+1,+1,+1) -- production signature
sig_engine = eng.local_marginal_sigma(R, G, Tc, t, S_true_vec, 1)
...
lemma_err = float(np.abs(sig_engine - sig_manual).max())
print(f"max |engine - per-bond-factorized| = {lemma_err:.2e}")
...
print(f"min Delta(1->2) on 400x400 grid = {grid_min:.3e}  "
      f"(at b1={B1.flat[vals.argmin()]:.3f}, b2={B2.flat[vals.argmin()]:.3f})")
...
rand_min = float(worst)
print(f"min increment over 20000 random (K<=6) configs = {rand_min:.3e}")
...
seq_min = float(worst_full)
print(f"min per-record increment over 2000 random sequences = {seq_min:.3e}")

print()
ok = (lemma_err < 1e-12 and grid_min > -1e-6 and rand_min > -1e-5 and seq_min > -1e-4)
print(f"[{'PASS' if ok else 'FAIL'}] G7 Proposition C (bond factorization + increment positivity): "
      f"lemma={lemma_err:.1e}, grid_min={grid_min:.1e}, rand_min={rand_min:.1e}, "
      f"seq_min={seq_min:.1e}")
