...
\includegraphics[width=\linewidth]{fig3_histograms_v2.png}
\caption{}
\end{subfigure}
...
\textbf{c}, Distribution of the conditional current across branches at four times ($t_S = 0.4$): unimodal at zero before the transition ($t = 0.1$), spreading through mid-transition ($t = 0.5$), sharply bimodal at $\pm I_0$ after. The two peaks remain of equal weight at all times: the ensemble stays exactly T-symmetric (the unconditional result above); the breaking is per-history, not ensemble-level.}
