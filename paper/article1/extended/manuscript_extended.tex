...
\date{\small Extended version: July 7, 2026}
...
\begin{abstract}
\noindent
The microscopic laws of physics are invariant under time reversal, yet every observer records a fixed arrow of time. We study how decoherence fixes the orientation of a time-reversal-odd observable in an exactly solvable model: a three-site chiral ring, whose degenerate current-carrying doublet forms a T-odd pointer basis, coupled to a bath of recording qubits. The model admits closed forms for all bipartite entropies, and its two-branch structure reduces trajectory statistics to classical inference. We prove that the unconditional stochastic entropy production vanishes identically (the arrow is spontaneously broken, existing only relative to a branch) and that the branch-relative entropy production equals exactly the accumulated Kullback--Leibler distinguishability $D(t)$ of the environmental records. Both facts are instances of a model-independent theorem proven here (the Branch Arrow Theorem, Sec.~\ref{sec:BAT}): for any microreversible dynamics with a two-valued time-reversal-odd branch variable, monitored so that the backward record statistics in a branch is the forward statistics of its antipode, the unconditional entropy production vanishes trajectory by trajectory and the branch-relative one equals the accumulated record distinguishability. The laws carry no arrow; conditioned histories do, and the ring is the minimal exactly solvable instance. Numerically, chirality fixation locks to decoherence ($t_P/t_S$ saturating at $0.956\pm0.003$ for bath sizes $N\ge64$), while Darwinian redundancy lags by a factor of four: the observable fixes before it becomes objective. Branch polarization collapses onto a universal function of $D$ across protocols and couplings, with no detectable hysteresis under record degradation. Measuring the records converts the revocable arrow into a ratchet. Classicalization of records, not decoherence alone, makes the local arrow permanent.
\end{abstract}
...
\subsection{The statement is model-independent: the Branch Arrow Theorem}
\label{sec:BAT}

Nothing in Theorems~\ref{th:zero} and \ref{th:identity} uses the ring, and it is worth isolating exactly what they do use.

\begin{theorem}[Branch Arrow Theorem]\label{th:BAT}
Assume: (H1) a microreversible dynamics carrying a two-valued branch variable $s = \pm 1$ that the antiunitary $\Theta$ exchanges; (H2) record trajectories $\xi$ with conjugate $\tilde\xi$ (reversed order, T-odd data) satisfying the antipodal covariance $P_{\mathrm{fwd}}(\xi\,|\,{-s}) = P_{\mathrm{fwd}}(\tilde\xi\,|\,s)$, and a $\Theta$-imaged backward protocol, so that $P_{\mathrm{bwd}}(\tilde\xi\,|\,s) = P_{\mathrm{fwd}}(\xi\,|\,{-s})$ (the covariance expresses the T-oddness of the record datum; the $\Theta$-imaged protocol converts it into the second equality, which is the only one the proofs use); (H3) a symmetric branch prior. Then: (a) $\sigma[\xi] = \ln P_{\mathrm{fwd}}(\xi) - \ln P_{\mathrm{bwd}}(\tilde\xi) = 0$ for every trajectory $\xi$; (b) $\sigma_s[\xi] = \ln[P(\xi|s)/P(\xi|{-s})]$, so that
\[ \langle\sigma_s\rangle \;=\; D_{\mathrm{KL}}\big[P(\cdot|s)\,\big\|\,P(\cdot|{-s})\big] \;\ge\; 0, \]
reducing to the additive form of Theorem~\ref{th:identity} exactly when the records are conditionally independent given the branch.
\end{theorem}

\begin{proof}
(a) By (H2) and (H3), $P_{\mathrm{bwd}}(\tilde\xi) = \tfrac12\sum_s P_{\mathrm{fwd}}(\xi|{-s}) = P_{\mathrm{fwd}}(\xi)$, so the log-ratio vanishes pointwise. (b) $\sigma_s[\xi] = \ln P(\xi|s) - \ln P_{\mathrm{bwd}}(\tilde\xi|s) = \ln P(\xi|s) - \ln P(\xi|{-s})$ by (H2); its expectation under $P(\cdot|s)$ is the stated divergence. Conditional independence is nowhere required; it only converts the divergence into the per-record sum.
\end{proof}

\begin{corollary}[T-even control]\label{cor:teven}
If the conjugation fixes the branch instead of exchanging it (a T-even pointer), then $P_{\mathrm{bwd}}(\tilde\xi|s) = P_{\mathrm{fwd}}(\xi|s)$ and $\sigma_s \equiv 0$ identically, while the marginal distinguishability of the branches may grow without bound. The failure of the identity for T-even pointers, noted after Theorem~\ref{th:identity}, is this corollary.
\end{corollary}

The essential ingredients are therefore three: a T-odd two-valued pointer, branch-resolving records with the antipodal covariance, and a record-free (symmetric) initial condition. The three sites, the ring geometry, the coupling distribution and the bath size carry no weight; the chiral ring is the minimal exactly solvable instance. The theorem was verified as an independent gate (CHECK~7 of the verification suite, repository) on randomly drawn \emph{correlated} record laws satisfying (H1)--(H3): conclusions (a) and (b) hold at machine precision, the additivity gap reaches $1.2$ nats for correlated laws and vanishes for product laws, and the T-even control gives $\sigma_s \equiv 0$ while $D$ grows. This is what licenses reading the mechanism as a principle: wherever a time-reversal-odd alternative is einselected by a recording environment, its fixation \emph{is} the local arrow, in the exact quantitative sense of Theorem~\ref{th:identity}.
...
\end{subfigure}\hfill
\begin{subfigure}[t]{0.49\linewidth}
\includegraphics[width=\linewidth]{fig2_cotransition_v2.png}
\caption{}
\end{subfigure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\linewidth]{fig3_histograms_v2.png}
\caption{\textbf{Spontaneous T-symmetry breaking at branch level.} Distribution of the conditional current across sampled records at four times ($t_S = 0.4$): unimodal at zero before the transition ($t = 0.1$), spreading through mid-transition ($t = 0.5$), sharply bimodal at $\pm I_0$ after. The two peaks remain of equal weight at all times: the ensemble stays exactly T-symmetric (Theorem~\ref{th:zero}); the breaking is per-history, not ensemble-level.}
\label{fig:hist}
\end{figure}
...
\bibitem{Zwolak2009} M.~Zwolak, H.~T. Quan, W.~H. Zurek, Quantum Darwinism in a mixed environment, \emph{Phys. Rev. Lett.} \textbf{103}, 110402 (2009).
